...
function FBXModel({ scrollProgress }: FBXModelProps) {
  ...
  useFrame(() => {
    ...
    if (lightRef.current) {
      ...
      const rotation = scrollProgress * Math.PI * 2; // Full rotation
      lightRef.current.position.set(Math.sin(rotation) * 10, 10, Math.cos(rotation) * 10);
  ...
  return (
    <>
      ...
      <directionalLight ref={lightRef} castShadow position={[1, 1, 1]} intensity={50} color={'#ffd700'} />
    </>
  );
}
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', zIndex: -1 }}>
      <Canvas>
        ...
        <directionalLight castShadow position={[0, -0.5, -1]} intensity={30} color={'#dbb549'} />
        <directionalLight castShadow position={[-1, 1, -1.5]} intensity={30} color={'#dbb549'} />
        <directionalLight castShadow position={[1, -1, -1.5]} intensity={30} color={'#dbb549'} />
        <directionalLight castShadow position={[0.5, 0.5, 2]} intensity={30} color={'#dbb549'} />
